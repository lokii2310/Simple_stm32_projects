...
void GPIO_IRQPriorityConfig(uint8_t IRQNumber, uint32_t IRQPriority)
{
    // 1. First, find the IPR register and section
    uint8_t iprx = IRQNumber / 4;  // IPR register
    uint8_t iprx_section = IRQNumber % 4;  // Section within the IPR register

    // 2. Calculate the shift amount
    uint8_t shift_amount = (8 * iprx_section) + (8 - NO_PR_BITS_IMPLEMENTED);

    // 3. Set the priority, first clear the section and then set the priority
    volatile uint32_t* ipr_base = (volatile uint32_t*)NVIC_PR_BASE_ADDR;
    ipr_base[iprx] &= ~(0xFF << (8 * iprx_section));  // Clear the section
    ipr_base[iprx] |= (IRQPriority << shift_amount);  // Set new priority
}
...
void GPIO_IRQHandling(uint8_t PinNumber)
{
    ...
    if (EXTI->PR1 & (1 << PinNumber))
    {
        // Clear the EXTI PR1 register by writing a '1' to the corresponding bit
        EXTI->PR1 = (1 << PinNumber);
    }
}
